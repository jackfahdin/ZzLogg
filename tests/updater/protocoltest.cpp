...
HandoffSession atStage(int stage) {
    ...
    constexpr MessageKind sequence[]={MessageKind::Hello,MessageKind::AwaitingAppExit,MessageKind::CommitExit,MessageKind::Complete};
...
int main() {
    ...
    const EncodedMessage fixture={0x5a,0x5a,0x55,0x50, 1,0,0,0, 1,0,0,0,
    ...
    constexpr HandoffState states[]={HandoffState::Connecting,HandoffState::Ready,
        HandoffState::AwaitingAppExit,HandoffState::ExitCommitted,HandoffState::Complete};
    constexpr uint32_t expected[]={1,3,4,7,0};
    for(int stage=0;stage<=4;++stage) {
        ...
        for(uint32_t kind=0;kind<=8;++kind) {
            ...
            const bool allowed=kind==expected[stage] && stage<4;
            check(accepted==(allowed || (cancel && stage<4)),"sequence transition acceptance");
        ...
        for(bool otherTransaction:{false,true}) {
            auto test=atStage(stage); auto wrong=message(stage<4?static_cast<MessageKind>(expected[stage]):MessageKind::Cancel);
    ...
    for(auto terminal:{MessageKind::Cancel,MessageKind::Failed}) for(uint32_t replay=1;replay<=7;++replay) {
