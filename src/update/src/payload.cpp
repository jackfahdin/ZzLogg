...
bool allowedUrl(std::string_view url,const std::vector<std::string>& hosts) {
...
Manifest parsePayload(const Json& json,const VerificationContext& context) {
    ...
    for(const auto& item:artifacts) {
        ...
        artifact.url=string(item.at("url")); require(allowedUrl(artifact.url,context.allowedHosts));
