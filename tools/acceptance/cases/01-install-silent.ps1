...
# PASS 判据：`setup.exe /S` 退出 0；`$InstallDir\ZzLogg.exe`、`.zzlogg-install-root`、
# `.zzlogg-files.manifest` 存在；manifest 首行 schema=2。
# 协议映射说明：落地清单是二进制格式（GenerateNsisManifest.ps1 与 txengine_win.cpp
...
function Invoke-Case_01_install_silent {
    ...
    try {
        ...
        if ($install.Run.TimedOut) {
            return (Fail-Case "setup.exe /S 超时（$($target)），进程被终止")
        }
        if ($install.Run.ExitCode -ne 0) {
            return (Fail-Case "setup.exe /S 退出码 $($install.Run.ExitCode)，期望 0")
        }
        ...
        return (Pass-Case "setup.exe /S 退出 0；ZzLogg.exe/.zzlogg-install-root/.zzlogg-files.manifest 均存在；manifest magic=ZZTXMAN1 version=1（schema 2 落地清单，schema 身份由 UpdateIdentitySchema=2 承载，见用例 02）")
    }
    finally {
        [void](Invoke-ZzLoggUninstall $Context)
