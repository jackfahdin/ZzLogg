...
- [ ] 运行现有行为测试，记录失败项，不把原有问题归因于重构。
- [ ] 为菜单访问引入一个测试辅助函数：优先查找 `zzloggFluentTitleBar`，再取其 `menuBar()`；原生测试窗口则使用 `window.menuBar()`。
- [ ] 保持动作名称、快捷键、选中状态、最后一个文件关闭以及语言切换断言。
...
- [ ] 新建 `zzlogg_ui2.window_chrome` 测试，断言创建组件时菜单栏为空且容器由框架标题栏拥有。
- [ ] 注入可失败的代理配置函数，覆盖附着失败、配置失败、成功三条路径；实现前运行测试确认缺少功能。
- [ ] 复用原外观代码中的能力判断、置顶、图标、标题同步和按钮意图连接，去掉菜单迁移算法。
- [ ] 成功时先配置代理，再由 `setMenuWidget()` 接管标题栏；返回标题栏菜单容器。
- [ ] 失败时释放代理和临时标题栏，恢复窗口 flags，选择原生菜单容器；记录错误，窗口仍能操作和关闭。
- [ ] 将标题格式化函数移入新组件文件，保留 Unicode 和长文件名测试。
- [ ] 测试重复开关窗口、关闭事件被拒绝、最大化/还原、主题切换，以及销毁后延迟事件不访问旧指针。
...
- [ ] 增加带 `UiThemeContext` 的构造路径，并抽取共用初始化方法，避免两套构造函数复制业务初始化。
- [ ] 在创建动作、菜单之前创建 `WindowChrome` 并确定菜单容器。
- [ ] 将 `createMenus()` 改为 `createMenus(QMenuBar& menuBar)`，完整迁移现有菜单项，所有父对象从该容器取得。
- [ ] 审计 `mainwindow.cpp` 中每个 `menuBar()` 调用；框架窗口不允许通过基类方法创建新的默认菜单栏。
- [ ] 把菜单与工具栏构造实现移到对应文件，动作仍共享原有 `QAction`；状态字段仍由 `MainWindow` 管理。
- [ ] 用真实生产构造路径验证编码菜单弹出、快捷键、工具栏选中状态、最近文件和收藏动态更新。
...
- [ ] 实现 `setMainWindowFactory()`，在 `newWindow()` 中选择已注册工厂或原生测试构造，保持原有窗口注册和信号连接顺序。
- [ ] 从旧运行时迁入主题初始化和旧 `System` 配置迁移逻辑，注册创建带上下文窗口的工厂。
- [ ] 主题服务创建完成后才允许新建生产窗口；退出时关闭窗口、清除工厂、释放样式对主题的引用，最后释放主题服务。
- [ ] 标题栏和设置仅请求主题变更，运行时统一持久化与应用；保持多窗口同步，避免两个控制器各自写配置。
- [ ] 沿用 `AbstractLogView::changeEvent()` 缓存刷新与工具栏图标重载，验证主日志、结果区、选区和禁用动作在深浅色下可读。
- [ ] 改写 `runtimecontracttest.cpp` 的内部访问，覆盖新窗口和会话恢复窗口都经过工厂。
...
- [ ] 在 `src/ui/CMakeLists.txt` 定义独立 `zzlogg_ui_runtime` 目标，将应用链接从 `zzlogg_ui2_runtime` 替换为新目标。
- [ ] 移除 `src/CMakeLists.txt` 中 `ui2` 子目录注册；保留测试目录与 preset 的兼容名字。
- [ ] 将 `klogg_copy_ui2_runtime_dlls` 改为中性名称并更新全部调用者，保持实际 DLL 复制集合不变。
- [ ] 迁移旧 shell 中仍有效的行为测试；删除只验证已取消的菜单迁移/事务回滚的测试，新组件失败测试替代它们。
- [ ] 审计旧运行时、私有头文件、友元测试和诊断属性引用，确保不会留下新旧双路径。
- [ ] 无调用者后删除旧源目录，更新学习指南与应用启动冒烟测试的结构断言。
...
- [ ] 使用本机现有工具链配置并构建 Release，新测试目标与应用一起构建。
- [ ] 执行阶段一基线测试、新 `window_chrome` 测试及完整已启用测试集；逐项处理新增失败。
- [ ] 为自动测试使用独立配置目录和现有 smoke 隔离机制，不让测试写用户设置或真实会话。
- [ ] 生成带依赖的运行目录；调用会清理目录的打包目标前核对目标内没有用户数据，必要时选择新构建输出目录。
- [ ] 在真实 Windows 窗口检查标题居中、菜单弹出/键盘操作、窄窗口自适应、拖动缩放、系统按钮、深浅色与中英文切换。
- [ ] 打开两个日志并搜索，关闭最后一个文件后再次打开，验证多窗口、退出和会话恢复。
...
- [ ] 生产窗口直接创建框架标题栏与菜单，布局和核心交互符合原界面。
- [ ] `src/ui2` 已删除，应用 UI 位于 `src/ui`，运行时与业务 UI 构建依赖无环。
- [ ] 原日志页面可用，主题、语言、窗口销毁与会话回归通过。
- [ ] 提供可运行预览和更新后的学习文档。
