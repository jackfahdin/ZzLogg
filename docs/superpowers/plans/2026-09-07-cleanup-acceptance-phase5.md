...
旧新版单文件大日志对照现已完成，报告记录了轻微滚动回退信号，尚未定位原因，不能宣称性能全面通过。Windows 人工多屏、不同缩放比例、拖动与吸附及设置窗口 150% 缩放已由用户确认正常。Linux/macOS 继续暂停，不作为当前阻塞项。详见 [验收收尾计划](2026-09-14-ui-acceptance-followup.md)。下面的“未合并”和 XFAIL 描述仅是早期历史记录，不是当前状态。
...
- [x] 旧版本对照的指定大文件打开、滚动、搜索、内存占用测量（2026-09-14，详见报告；不代表性能全面通过）。
- [ ] 剖析对照发现的轻微滚动重绘回退信号。
- [x] 人工多屏、不同缩放比例、拖动与吸附检查（2026-09-14 用户确认正常，含设置窗口 150% 缩放验证）。
