...
**方案：** 5 个工作流：CI Build（PR/手动/复用构建）、Publish（tag/定时/手动发布）、Signed Update Feed、CodeQL（每周/手动）、Update Smoke（相关文件变化/手动）。发布前仍校验固定源提交、完整产物和正式版标签；不取消正在执行的旧版发布。
...
- [x] 更新清单监听 Publish；迁移期间兼容仍在运行的 Release / Continuous Build，之后移除旧名称。
- [x] 更新文档，执行 actionlint、53 项 Python 发布/签名回归及独立审查，提交推送后核实普通 master push 不触发全平台构建（上线验证待跟踪）。
