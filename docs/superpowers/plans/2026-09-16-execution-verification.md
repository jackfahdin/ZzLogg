...
- [ ] 补充文件稳定性边界：存在可写映射（即使创建映射的文件句柄已关闭）时打开必须失败。祖先目录不仅阻止重命名，还须证明无法改为 junction；测试 GENERIC_WRITE 以及 FILE_WRITE_ATTRIBUTES 访问下的实际修改尝试，不能把拒绝普通写句柄等同于拒绝全部元数据操作。保持各级子对象不被移走使祖先非空，配合 Windows 对非空目录设置重解析点的拒绝；优先只共享读，任何兼容性放宽须用实际攻击回归验证。
