...
> v26.09.02 安装器迁移：当前入口为 `packaging/windows/ZzLogg.iss`，使用 Inno Setup 7.1.0。本文阶段记录中的 NSIS 名称及具体脚本 API 描述保留历史含义；现行静默参数为 `/VERYSILENT /SUPPRESSMSGBOXES /NORESTART`，受限入口同时禁止 `/DIR=` 和旧 `/D=` 覆盖。清单中的 `nsis-exe` 继续作为兼容旧客户端的协议枚举，二进制落地清单、schema 2 身份、定位名和退出码不变。从 v26.09.03 起，稳定版 Windows x64 注册安装可在 Ed25519 清单锚定下走应用内「退出并更新」；项目仍无 Authenticode，UAC 会显示未知发布者。阶段 4 实机验收与部分 Inno 卸载日志事务项仍待完成。
...
**生产在线更新已在 GitHub 签名清单上运行**（稳定版与预览版渠道、Windows/macOS/Linux 检查与下载）。Windows 稳定版 x64 注册安装自 v26.09.03 起支持验签后的应用内一键安装与失败回滚；便携版仍手动更新，预览渠道仍主要依赖发布页手动下载。当前只支持全量包下载，不支持增量更新。范围及历史阶段见 [仅安装版在线更新设计](../superpowers/specs/2026-09-15-installer-only-update-design.md)。
...
正式发布身份只是用于比较的元数据，**不等于安装授权**。检查与下载路径已接入组合后的 `InstalledRelease`；Windows 稳定版在正式发布身份、注册安装根目录与 `verifyPackageForExecution` 通过后，生产会话工厂才会开放「退出并更新」。便携版、预览渠道或未满足开闸条件的安装仍不能启动安装器或写入 HKLM。
...
注册表，不信任以前的 `verifiedPath` 或缓存成功状态。本项目暂无代码签名证书：执行信任根是
Ed25519 签名清单给出的包大小与 SHA-256；发布者指纹列表为空时跳过 Authenticode，
非空时在清单锚定之上叠加发布者钉扎。
...
满足生产会话输入（正式发布身份、注册安装根、已验签包路径/大小/摘要）时开放真实安装。
...
- `ApplicationUpdateHandoff`（Qt 控制器）按 Idle→Preparing→Waiting→ExitCommitted
  ...
  恢复窗口，待对端取消完成后再释放预留。**执行可用性**由 `makeUpdateSessionFactory`
  是否非空决定（需正式发布身份、注册安装、已验签包元数据及同目录 `ZzLoggUpdate.exe`）；
  只有活着且已认证的 AwaitingAppExit 对端才允许提交退出；提交后由宿主显式安排进程
  退出（退出码 0，绝不用重启码 773）。
- 预留跨越主程序真实退出：bootstrap（版本 4）可选携带预留目录身份，子协调者在
...
- 更新对话框的「退出并更新」受上述生产能力约束：工厂为空时按钮隐藏；检查/选择/下载
  变化都会清除已推入的能力。准备/等待期间检查、下载、跳过等改变选择的动作停用，
  关闭/ESC 等价取消。英/简/繁运行时切换与长文本布局有自动验收。便携副本仍仅提示手动更新。
- `ZzLoggUpdate.exe` 通过正式 CMake target 进入安装产物与 Windows runtime folder
  ...
  稳定版一键安装仍缺 Windows Authenticode 与阶段 4 全量实机签字；同目录其他实例
