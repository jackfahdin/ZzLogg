...
**这不代表生产在线更新已上线。** 正式托管、公钥和发布身份尚未配置；便携版替换、安装器执行和失败回滚属于后续阶段。只支持全量包，不支持增量更新或自动安装。
...
下载、缓存和校验后端已实现；解压路径检查、文件替换事务、安装器提权、回滚及发布签名工具仍未实现。后续实现便携版与安装版执行器，再建设真实发布流水线。包已下载不代表可以执行或安装。
