...
调用方只能在 verifyManifest 返回 value 后持久化其中的接受记录。结构合法但没有适用载荷的清单仍需保存接受记录，不能因为 selectUpdate 未发现新版就丢弃。写入应当原子完成，并且绑定正确渠道；存储实现属于下一阶段。
...
Windows 验收包含旧 64 项测试与新增更新测试。跨平台实际运行未验证，不宣称已通过。

本阶段没有解压路径检查、文件替换事务、安装器提权、回滚或发布签名工具。下一阶段接入 Qt 网络、检查更新 UI 和渠道状态持久化；随后分别实现便携版与安装版执行器，再建设真实发布流水线。
